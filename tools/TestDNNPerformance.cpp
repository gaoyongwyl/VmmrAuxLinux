...
{
	printf( "Usage of %s\n", szAppName );
	printf( "%s <NNProtoFile> <NNBinaryModel> <SampleFolder> <ListFile> <<featName1>:<featName2>; ..<featNamen>> <classLayerName> <classType> <workFolder> <IsShowResultImage> <ComputMod>\n", szAppName );
...
int main(int argc, char* argv[])
{
        printf( "argc = %d\n", argc );
	if( argc < 12 ) {
		cout << endl << "No enough input parameters !" << endl << endl;
		Usage( argv[0] );
		exit(-1);
	}
	...
	ClassificationTest( pDnnFeatureBoy, strClassLayerName, enVRClassType, strSampleFolder, strListFile, strWorkFolder, bShowResultImages );
